...
let prefix = "x/"
 try { 
   ...
   if(!message.member.hasPermission("MANAGE_MESSAGES")) return message.reply("But.. How can you do this without permissions? You need these permissions to execute the command! **\`MANAGE_MESSAGES\`** 😕");
  if(!message.guild.me.hasPermission("MANAGE_MESSAGES")) return message.reply("Oh! Apparently I do not have the following permissions to delete the messages from the channel! **\`MANAGE_MESSAGES\`**");
...
if(!amount || amount < 2 || amount > 200) return message.channel.send("I think you should locate a number from 1 to 100, without that I can't help you!")
if(!args[1]) {
...
await message.channel.bulkDelete(amount).then(async (m) => { 
  ...
  .setColor('#EDEDFA')
  .setDescription(`<:check_icon:891849824650018876> Deleted messages and clean channel! | Deleted messages : **${m.size}**/**${amount}**`);
   ...
   message.channel.send(embed).then(msg => msg.delete({timeout:4000})); 
     ...
     console.log(e) 
     message.channel.send(`But what happens here? I cannot delete messages that have been on this channel for more than 12 days! 😅`)
